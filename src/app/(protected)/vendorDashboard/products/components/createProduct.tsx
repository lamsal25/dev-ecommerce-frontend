...
export default function CreateProduct({ onSuccess }: CreateProductProps) {
  ...
  const form = useForm<ProductFormValues>({
    ...
    defaultValues: {
      ...
      discountedPrice: 0,
      isAvailable: true,
      totalStock: 1, // Changed from 0 to 1 as default
      has_sizes: false,
      sizes: [],
    },
  });
  ...
  const resetForm = useCallback(() => {
    form.reset({
      ...
      discountedPrice: 0,
      isAvailable: true,
      totalStock: 1, // Changed from 0 to 1
      has_sizes: false,
      sizes: [],
    });
    setImageFields({
  ...
  const validateForm = useCallback((values: ProductFormValues) => {
    ...
    return true;
  }, [imageFields.image, calculateTotalStock]);
  ...
  const imageConfigs = [
    { key: "image", label: "Main Product Image", required: true },
      ...
      <SheetContent className="overflow-y-auto w-full max-w-md sm:max-w-lg px-6 pb-8">
        <SheetHeader>
          ...
          <SheetTitle className="text-2xl font-semibold text-center">
            Create <span className="text-secondary underline">New Product</span>
          </SheetTitle>
        </SheetHeader>
